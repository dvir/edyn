...
If the new entity doesn't connect to any other entity which is already in an island, a new island is created for it, and all nodes and edges connected to it are also inserted in the new island.

If the new entity connects to an existing entity which is already in an island, then it is inserted into that island.

If the new entity connects to entities in different islands, these islands have to be merged into one. The biggest island is selected and the entities in the other islands are inserted into it. The smaller islands can be destroyed right after.
...
Generally, the `edyn::msg::share_islands` asks the worker to consider sending some of its islands to another worker, be it to balance load or to make sure collisions are not missed.
...
Ideally, the rigid bodies in an island worker should have close proximity. For example, in a simulation where there's a flat ground plane and over a hundred boxes scattered all over it, where islands have 1 or 2 boxes each, it is desirable to have groups of boxes that are near each other to be located in one island worker. The worker should prefer groups of islands that are closer to the islands in the destination worker when balancing load.
